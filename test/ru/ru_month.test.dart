...
void main() {
  ...
  test("Test - Month-Only expression", () {
    testSingleCase(ru.casual, "в январе", new Date(2020, 11 - 1, 22), (result) {
      expect(result.index).toBe(0);
      expect(result.text).toBe("в январе");
      expect(result.start).toBeDate(new Date(2021, 1 - 1, 1, 12));
    });
    testSingleCase(ru.casual, "в янв", new Date(2020, 11 - 1, 22), (result) {
      expect(result.index).toBe(0);
      expect(result.text).toBe("в янв");
      expect(result.start).toBeDate(new Date(2021, 1 - 1, 1, 12));
    });
    testSingleCase(ru.casual, "май", new Date(2020, 11 - 1, 22), (result) {
      expect(result.index).toBe(0);
  ...
  test("Test - year 90's parsing", () {
    testSingleCase(ru.casual, "авг 96", new Date(2012, 7, 10), (result) {
      expect(result.index).toBe(0);
      expect(result.text).toBe("авг 96");
      expect(result.start).toBeDate(new Date(1996, 8 - 1, 1, 12));
    });
    testSingleCase(ru.casual, "96 авг 96", new Date(2012, 7, 10), (result) {
