...
void main() {
  test("Test - Single expression", () {
    testSingleCase(ru.casual, "10.08.2012", new Date(2012, 7, 10), (result) {
      expect(result.index).toBe(0);
      expect(result.text).toBe("10.08.2012");
      expect(result.start).toBeDate(new Date(2012, 8 - 1, 10, 12));
    });
    testSingleCase(ru.casual, "10 августа 2012", new Date(2012, 7, 10),
        (result) {
      expect(result.index).toBe(0);
      expect(result.text).toBe("10 августа 2012");
      expect(result.start).toBeDate(new Date(2012, 8 - 1, 10, 12));
    });
    testSingleCase(ru.casual, "третье фев 82", new Date(2012, 7, 10), (result) {
      expect(result.index).toBe(0);
      expect(result.text).toBe("третье фев 82");
      expect(result.start).toBeDate(new Date(1982, 2 - 1, 3, 12));
    });
    testSingleCase(ru.casual, "Дедлайн 10 августа", new Date(2012, 7, 10),
        (result) {
    ...
    testSingleCase(
        ru.casual, "Дедлайн Четверг, 10 января", new Date(2012, 7, 10),
        (result) {
      expect(result.index).toBe(8);
  ...
  test("Test - Single expression with separators", () {
    testSingleCase(ru.casual, "10-августа 2012", new Date(2012, 7, 8),
        (result, text) {
  ...
  test("Test - Range expression", () {
    testSingleCase(ru.casual, "10 - 22 августа 2012", new Date(2012, 7, 10),
        (result) {
      expect(result.index).toBe(0);
    ...
    testSingleCase(ru.casual, "с 10 по 22 августа 2012", new Date(2012, 7, 10),
        (result) {
      expect(result.index).toBe(0);
    ...
    testSingleCase(ru.casual, "10 августа - 12 сентября", new Date(2012, 7, 10),
        (result) {
      expect(result.index).toBe(0);
    ...
    testSingleCase(
        ru.casual, "10 августа - 12 сентября 2013", new Date(2012, 7, 10),
        (result) {
      expect(result.index).toBe(0);
  ...
  test("Test - Combined expression", () {
    testSingleCase(ru.casual, "5 мая 12:00", new Date(2012, 7, 10), (result) {
      expect(result.index).toBe(0);
  ...
  test("Test - Ordinal Words", () {
    testSingleCase(ru.casual, "двадцать пятое мая", new Date(2012, 1, 10),
        (result) {
      expect(result.index).toBe(0);
    ...
    testSingleCase(
        ru.casual, "двадцать пятое мая 2020 года", new Date(2012, 1, 10),
        (result) {
      expect(result.index).toBe(0);
  ...
  test("Test - little endian date followed by time", () {
    testSingleCase(
        ru.casual, "24го октября, 9:00", new Date(2017, 7 - 1, 7, 15),
        (result) {
      expect(result.index).toBe(0);
  ...
  test("Test - year 90's parsing", () {
    testSingleCase(ru.casual, "03 авг 96", new Date(2012, 7, 10), (result) {
      expect(result.index).toBe(0);
  ...
  test("Test - Forward Option", () {
    testSingleCase(ru.casual, "22-23 фев в 7", new Date(2016, 3 - 1, 15),
  ...
  test("Test - Impossible Dates (Strict Mode)", () {
    testUnexpectedResult(
        ru.strict, "32 августа 2014", new Date(2012, 7, 10));
    testUnexpectedResult(
        ru.strict, "29 февраля 2014", new Date(2012, 7, 10));
    testUnexpectedResult(ru.strict, "32 августа", new Date(2012, 7, 10));
    testUnexpectedResult(ru.strict, "29 февраля", new Date(2013, 7, 10));
  });
}
