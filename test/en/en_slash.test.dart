 ...
 void main() {
  ...
  test("Test - Single Expression", () {
   testSingleCase(
    ...
    testSingleCase(
      ...
      expect(result.start).toBeDate(DateTime(2015, 10, 3, 12));
    });
    testSingleCase(en.casual.strict, "2/28/2014",
        (ParsedResult result, String text) {
      expect(result.text, "2/28/2014");
    });
    testWithExpectedDate(
        en.casual.strict, "12-30-16", DateTime(2016, 12 - 1, 30, 12));
    testSingleCase(en.casual.strict, "Friday 12-30-16",
        (ParsedResult result, String text) {
      expect(result.text, "Friday 12-30-16");
      expect(result).toBeDate(DateTime(2016, 12 - 1, 30, 12));
    });
  });
  test("Test - Single Expression Little-Endian", () {
    testSingleCase(en.casual.en.GB, "8/10/2012", DateTime(2012, 7, 10),
      ...
      expect(result.index, 0);
      expect(result.text, "8/10/2012");
      expect(result.start).toBeDate(DateTime(2012, 10 - 1, 8, 12));
    });
    testWithExpectedDate(
        en.casual.strict, "30-12-16", DateTime(2016, 12 - 1, 30, 12));
    testSingleCase(en.casual.strict, "Friday 30-12-16",
        (ParsedResult result, String text) {
  ...
  test("Test - Range Expression", () {
   testSingleCase(en.casual.en, "8/10/2012 - 8/15/2012", DateTime(2012, 7, 10),
        (ParsedResult result, String text) {
      expect(result.index, 0);
      ...
      expect(result.end.get("day"), 15);
      expect(result.end).toBeDate(DateTime(2012, 8 - 1, 15, 12));
    });
  });
  ...
  test("Test - forward dates only option", () {
   testSingleCase(
        ...
        (ParsedResult result, String text) {
      expect(result.start == null, isFalse);
      expect(result.start.get("year"), 2000);
      ...
      expect(result.start.isCertain("month"), true);
      expect(result.start.isCertain("year"), false);
      expect(result.start).toBeDate(DateTime(2000, 5 - 1, 31, 12));
    });
    testSingleCase(en.casual, "1/8 at 12pm", DateTime("Sep 25 2021 12:00:00"),
      ...
      expect(result.start.get("day"), 8);
      expect(result.start).toBeDate(DateTime(2022, 1 - 1, 8, 12));
    });
  });
 }