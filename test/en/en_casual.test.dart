...
import '../test_util.dart';
 ...
 void main() {
  test("Test - Single Expression", () {
   testSingleCase(en.casual, "The Deadline is now", new Date (
       2012,
       7,
       10,
       8,
   ...
   testSingleCase(
       en.casual.casual, "The Deadline is now, without implicit local timezone",
   ...
   testSingleCase(
       en.casual.casual, "The Deadline is today", new Date (2012, 7, 10, 14, 12), (
       result) {
    expect(result.index).toBe(16);
   ...
   testSingleCase(
       en.casual.casual, "The Deadline is Tomorrow", new Date (2012, 7, 10, 1), (
       result) {
   ...
   testSingleCase(
       en.casual.casual, "The Deadline is midnight ", new Date (2012, 7, 10, 12), (
       result) {
  ...
  test("Test - Combined Expression", () {
   testSingleCase(
       en.casual.casual, "The Deadline is today 5PM", new Date (2012, 7, 10, 12), (
       result) {
    expect(result.index).toBe(16);
   ...
   testSingleCase(
       en.casual.casual, "Tomorrow at noon", new Date (2012, 8 - 1, 10, 14), (
       result) {
    expect(result.start.get("year")).toBe(2012);
    expect(result.start.get("month")).toBe(8);
    expect(result.start.get("day")).toBe(11);
  ...
  test("Test - Casual time implication", () {
   testSingleCase(en.casual.casual, "annual leave from today morning to tomorrow",
   ...
   testSingleCase(
       en.casual.casual, "annual leave from today to tomorrow afternoon",
       new Date (2012, 8 - 1, 4, 12), (result) {
  ...
  test("Test - Random text", () {
   testSingleCase(
       en.casual, "tonight", new Date (2012, 1 - 1, 1, 12), (result, text) {
   ...
   testSingleCase(
       en.casual, "tonight 8pm", new Date (2012, 1 - 1, 1, 12), (result, text) {
    expect(result.text).toBe(text);
    expect(result.start.get("hour")).toBe(20);
    expect(result.start.get("year")).toBe(2012);
    expect(result.start.get("month")).toBe(1);
   ...
   testSingleCase(
       en.casual, "tonight at 8", new Date (2012, 1 - 1, 1, 12), (result, text) {
    expect(result.text).toBe(text);
    expect(result.start.get("hour")).toBe(20);
    expect(result.start.get("year")).toBe(2012);
    expect(result.start.get("month")).toBe(1);
   ...
   testSingleCase(
       en.casual, "tomorrow before 4pm", new Date (2012, 1 - 1, 1, 12), (result,
       text) {
    expect(result.text).toBe(text);
   ...
   testSingleCase(
       en.casual, "tomorrow after 4pm", new Date (2012, 1 - 1, 1, 12), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("hour")).toBe(16);
    expect(result.start.get("year")).toBe(2012);
   ...
   testSingleCase(en.casual, "thurs", (result, text) {
    expect(result.text).toBe(text);
   ...
   testSingleCase(
       en.casual, "this evening", new Date (2016, 10 - 1, 1), (result, text) {
    expect(result.text).toBe(text);
   ...
   testSingleCase(
       en.casual, "yesterday afternoon", new Date (2016, 10 - 1, 1), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2016);
   ...
   testSingleCase(
       en.casual, "tomorrow morning", new Date (2016, 10 - 1, 1, 8), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2016);
    expect(result.start.get("month")).toBe(10);
   ...
   testSingleCase(
       en.casual, "this afternoon at 3", new Date (2016, 10 - 1, 1, 8), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2016);
    expect(result.start.get("month")).toBe(10);
  ...
  test("Test - Casual time with timezone", () {
   testSingleCase(en.casual, "Jan 1, 2020 Morning UTC", (result, text) {
    expect(result.text).toBe(text);
   ...
   testSingleCase(en.casual, "Jan 1, 2020 Evening JST", (result, text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2020);
    expect(result.start.get("month")).toBe(1);
    expect(result.start.get("day")).toBe(1);
