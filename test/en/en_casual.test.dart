 ...
 void main() {
  test("Test - Single Expression", () {
   testSingleCase(chrono.casual, "The Deadline is now", new Date (
       2012,
       7,
       10,
       8,
    ...
    expect(result.start).toBeDate(new Date (
        2012,
        7,
        10,
        8,
   ...
   testSingleCase(
       chrono.casual, "The Deadline is now, without implicit local timezone",
       { "instant": new Date (1637674343000), "timezone": null}, (result) {
    expect(result.text).toBe("now");
    expect(result.start).toBeDate(new Date (1637674343000));
    expect(result.start.isCertain("timezoneOffset")).toBe(false);
   });
   testSingleCase(
       chrono.casual, "The Deadline is today", new Date (2012, 7, 10, 14, 12), (
       result) {
    expect(result.index).toBe(16);
   ...
   testSingleCase(chrono.casual, "The Deadline is Tomorrow",
       new Date (2012, 7, 10, 17, 10), (result) {
        expect(result.index).toBe(16);
   ...
   testSingleCase(
       chrono.casual, "The Deadline is Tomorrow", new Date (2012, 7, 10, 1), (
       result) {
    expect(result.start).toBeDate(new Date (2012, 7, 11, 1));
   });
   testSingleCase(chrono.casual, "The Deadline was yesterday",
       new Date (2012, 7, 10, 12), (result) {
   ...
   testSingleCase(chrono.casual, "The Deadline was last night ",
       new Date (2012, 7, 10, 12), (result) {
        expect(result.index).toBe(17);
   ...
   testSingleCase(chrono.casual, "The Deadline was this morning ",
       new Date (2012, 7, 10, 12), (result) {
        expect(result.index).toBe(17);
   ...
   testSingleCase(chrono.casual, "The Deadline was this afternoon ",
       new Date (2012, 7, 10, 12), (result) {
        expect(result.index).toBe(17);
   ...
   testSingleCase(chrono.casual, "The Deadline was this evening ",
       new Date (2012, 7, 10, 12), (result) {
        expect(result.index).toBe(17);
   ...
   testSingleCase(
       chrono.casual, "The Deadline is midnight ", new Date (2012, 7, 10, 12), (
       result) {
   ...
   testSingleCase(chrono.casual, "The Deadline was midnight ",
       new Date (2012, 8 - 1, 10, 1), (result) {
        expect(result.text).toBe("midnight");
        expect(result.start.get("year")).toBe(2012);
        expect(result.start.get("month")).toBe(8);
        expect(result.start.get("day")).toBe(10);
   ...
   testSingleCase(chrono.casual, "The Deadline was midnight ",
       new Date (2012, 8 - 1, 10, 1), { "forwardDate": true}, (result) {
        expect(result.text).toBe("midnight");
        expect(result.start.get("year")).toBe(2012);
        expect(result.start.get("month")).toBe(8);
        expect(result.start.get("day")).toBe(11);
  ...
  test("Test - Combined Expression", () {
   testSingleCase(
       chrono.casual, "The Deadline is today 5PM", new Date (2012, 7, 10, 12), (
       result) {
    expect(result.index).toBe(16);
   ...
   testSingleCase(
       chrono.casual, "Tomorrow at noon", new Date (2012, 8 - 1, 10, 14), (
       result) {
    expect(result.start.get("year")).toBe(2012);
    expect(result.start.get("month")).toBe(8);
    expect(result.start.get("day")).toBe(11);
  ...
  test("Test - Casual date range", () {
   testSingleCase(chrono.casual, "The event is today - next friday",
  ...
  test("Test - Casual time implication", () {
   testSingleCase(chrono.casual, "annual leave from today morning to tomorrow",
       new Date (2012, 8 - 1, 4, 12), (result) {
   ...
   testSingleCase(
       chrono.casual, "annual leave from today to tomorrow afternoon",
       new Date (2012, 8 - 1, 4, 12), (result) {
  ...
  test("Test - Random text", () {
   testSingleCase(
       chrono, "tonight", new Date (2012, 1 - 1, 1, 12), (result, text) {
   ...
   testSingleCase(
       chrono, "tonight 8pm", new Date (2012, 1 - 1, 1, 12), (result, text) {
    expect(result.text).toBe(text);
    expect(result.start.get("hour")).toBe(20);
    expect(result.start.get("year")).toBe(2012);
    expect(result.start.get("month")).toBe(1);
   ...
   testSingleCase(
       chrono, "tonight at 8", new Date (2012, 1 - 1, 1, 12), (result, text) {
    expect(result.text).toBe(text);
    expect(result.start.get("hour")).toBe(20);
    expect(result.start.get("year")).toBe(2012);
    expect(result.start.get("month")).toBe(1);
   ...
   testSingleCase(
       chrono, "tomorrow before 4pm", new Date (2012, 1 - 1, 1, 12), (result,
       text) {
    expect(result.text).toBe(text);
   ...
   testSingleCase(
       chrono, "tomorrow after 4pm", new Date (2012, 1 - 1, 1, 12), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("hour")).toBe(16);
    expect(result.start.get("year")).toBe(2012);
   ...
   testSingleCase(
       chrono, "this evening", new Date (2016, 10 - 1, 1), (result, text) {
    expect(result.text).toBe(text);
   ...
   testSingleCase(
       chrono, "yesterday afternoon", new Date (2016, 10 - 1, 1), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2016);
   ...
   testSingleCase(
       chrono, "tomorrow morning", new Date (2016, 10 - 1, 1, 8), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2016);
    expect(result.start.get("month")).toBe(10);
   ...
   testSingleCase(
       chrono, "this afternoon at 3", new Date (2016, 10 - 1, 1, 8), (result,
       text) {
    expect(result.text).toBe(text);
    expect(result.start.get("year")).toBe(2016);
    expect(result.start.get("month")).toBe(10);
   ...
   testSingleCase(chrono.casual, "at midnight on 12th August",
       new Date (2012, 8 - 1, 10, 15), (result) {
        expect(result.start.get("year")).toBe(2012);
        expect(result.start.get("month")).toBe(8);
