...
void main() {
  test("Test - Date + Time Expression", () {
    testSingleCase(en.casual, "Something happen on 2014-04-18 13:00 - 16:00 as",
      ...
      expect(result.start).toBeDate(DateTime(2014, 4 - 1, 18, 13));
      expect(result.end).toBeDate(DateTime(2014, 4 - 1, 18, 16));
    });
  });
  test("Test - Time Expression", () {
    testSingleCase(en.casual, "between 3:30-4:30pm", DateTime(2020, 7 - 1, 6),
      ...
      expect(result.start).toBeDate(DateTime(2020, 7 - 1, 6, 15, 30));
      expect(result.end).toBeDate(DateTime(2020, 7 - 1, 6, 16, 30));
    });
    testSingleCase(en.casual, "9:00 PST", DateTime(2020, 7 - 1, 6),
      ...
      expect(result.start.get("timezoneOffset"), -480);
    });
  });
  test("Test - Quoted Expressions", () {
    ...
    testSingleCase(en.casual, "The date is '2014-04-18'",
      ...
      expect(result.start).toBeDate(DateTime(2014, 4 - 1, 18, 12));
    });
  });
  ...
  test("Test - Built-in English variants", () {
    testSingleCase(en.casual.en, "6/10/2018",
        (ParsedResult result, String text) {
      expect(result.start).toBeDate(DateTime(2018, 6 - 1, 10, 12));
    });
    testSingleCase(en.casual.en.GB, "6/10/2018",
        (ParsedResult result, String text) {
      expect(result.start).toBeDate(DateTime(2018, 10 - 1, 6, 12));
    });
  });
  test("Test - Random text", () {
    ...
    testSingleCase(en.casual, "9:00 AM to 5:00 PM, Tuesday, 20 May 2013",
      ...
      expect(result.start).toBeDate(DateTime(2013, 4, 20, 9, 0));
      expect(result.end).toBeDate(DateTime(2013, 4, 20, 17, 0));
    });
    testSingleCase(
      ...
      expect(result.end.get("day"), 7);
      expect(result.end.get("month"), 7);
    });
  ...
  test("Test - Customize by removing time extraction", () {
    ...
    testSingleCase(custom, "Thursday 9AM", DateTime(2020, 11 - 1, 29),
      ...
      expect(result.start.get("day"), 26);
    });
  });
}
